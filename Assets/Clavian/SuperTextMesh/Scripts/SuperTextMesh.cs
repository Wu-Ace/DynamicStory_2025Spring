...
*/
	...
	public Coroutine Read()
	{
		return Read(0f);
	}

	public Coroutine Read(float startTime)
	{
		StopReadRoutine();
...
//			alreadyInvoked.Clear();
			//lastNum = -1;
			latestNumber =
				-1; //reset to -1, not 0. This would stop first character from making a sound/playing an event (2018-05-29)
	...
	*/
	...
	private string FlipParagraphs(string myText, bool flipInfo)
	{
		...
		if(flipInfo)
		{
			...
			{
				Debug.Log("Something went wrong with the RTL system. Old info length was " + info.Count +
			...
			if(richText && myText[i] == '<')
			{
				...
				if(closingIndex != -1)
				{
					...
					switch(ParseText_myTag)
					{
						...
						//unicode
						case "<u=":
							insertAfter = char.ConvertFromUtf32(int.Parse(ParseText_myString,
...
//		if(uiMode){ //remove startup warnings
		...
		for(int i = 0, iL = hyphenedText.Length; i < iL; i++)
		{
			...
			Limits_info = info[i];
			Limits_font =
				Limits_info.fontData != null ? Limits_info.fontData.font : font; //use info's font, or default?
			Limits_font.RequestCharactersInTexture(
				hyphenedText[i].ToString(System.Globalization.CultureInfo.InvariantCulture),
				GetFontSize(Limits_font, Limits_info), Limits_info.ch.style); //request characters to draw
			//CharacterInfo ch;
			if(Limits_font.GetCharacterInfo(hyphenedText[i], out Limits_ch, GetFontSize(Limits_font, Limits_info),
				   Limits_info.ch.style))
			{
		...
		if(bestFit != BestFitMode.Off)
		{
			...
			if(bestFit == BestFitMode.MultilineBETA)
			{
				...
				*/
				...
				float smallestBestFitMulti = bestFitMulti;
				bestFitMulti = Mathf.Lerp(smallestBestFitMulti, BestFit_vertLimit / -unwrappedBottomRightTextBounds.y,
					0.1f);
				if(bestFitMulti > size) bestFitMulti = size; //keep below set size
				else if(bestFitMulti < smallestBestFitMulti)
					bestFitMulti = smallestBestFitMulti; //keep it above smallest size
	...
	void RebuildTextInfo()
	{
		...
		if(Rebuild_autoWrap > 0f)
		{
			...
			for(int i = 0, iL = hyphenedText.Length; i < iL; i++)
			{
				...
				Rebuild_font.RequestCharactersInTexture(
					hyphenedText[i].ToString(System.Globalization.CultureInfo.InvariantCulture),
					GetFontSize(Rebuild_font, Rebuild_info), Rebuild_info.ch.style);
				if(Rebuild_font.GetCharacterInfo(hyphenedText[i], out Rebuild_ch,
					   GetFontSize(Rebuild_font, Rebuild_info), Rebuild_info.ch.style))
				{
					//does this character exist?
			...
			for(int i = 0; i < infoCount; i++)
			{
				...
				Rebuild_font.GetCharacterInfo('\n', out Rebuild_breakCh, GetFontSize(Rebuild_font, Rebuild_info),
					style); //get data for linebreak
				//CharacterInfo hyphenCh;
				Rebuild_font.RequestCharactersInTexture("\u00AD", GetFontSize(Rebuild_font, Rebuild_info),
					style); //still call this, for when you're inserting hyphens anyway
				Rebuild_font.GetCharacterInfo('\u00AD', out Rebuild_hyphenCh, GetFontSize(Rebuild_font, Rebuild_info),
					style);
				Rebuild_font.GetCharacterInfo('\u200B', out Rebuild_zeroWidthCh,
					GetFontSize(Rebuild_font, Rebuild_info), style);
...
*/
					...
					if(!breakText && myActualBreak != -1 && myActualBreak > lastBreak)
					{
						...
						{
							hyphenedText =
								hyphenedText.Insert(myActualBreak + 1, "\u00AD\n"); //soft hyphen and linebreak
							...
							info[myActualBreak + 2].UpdateCachedValuesIfChanged(fontTextureJustRebuilt);
		...
		else
		{
			...
			for(int i = 0, iL = hyphenedText.Length; i < iL; i++)
			{
				...
				Rebuild_font.RequestCharactersInTexture(
					hyphenedText[i].ToString(System.Globalization.CultureInfo.InvariantCulture),
					GetFontSize(Rebuild_font, Rebuild_info), Rebuild_info.ch.style); //request characters to draw
	...
	void TrimCutoffText()
	{
...

		{
			...
			for(int i = 0; i < hyphenedText.Length; i++)
			{
				...
				else if(info[i].pos.y < autoPauseStopPoint - offset.y) //if this text is below the bounds... 
				{
			...
			else
			{
				...
				endVerts[4 * i + 0].x =
					((CurrentTextInfo.TopLeftVert.x + jitterValue.x + UpdateMesh_waveValueTopLeft.x +
					  UpdateMesh_waveValue.x) + UpdateMesh_lowestLineOffset.x + realBaseOffset.x);
				endVerts[4 * i + 0].y =
					((CurrentTextInfo.TopLeftVert.y + jitterValue.y + UpdateMesh_waveValueTopLeft.y +
					  UpdateMesh_waveValue.y) + UpdateMesh_lowestLineOffset.y + realBaseOffset.y);
				endVerts[4 * i + 0].z =
...
// 			textMesh.SetUVs(2, ArrayToList(endUv3));
	...
	void PrepareSubmeshes()
	{
		...
		{
			Submesh_sharedMaterial = SharedMaterialDataStorage.DoesSharedMaterialExist(this, null);
		...
		for(int i = 0, iL = hyphenedText.Length; i < iL; i++) //go thru all info
		{
			...
			if(Submesh_sharedMaterial == null) //doesn't exist yet??
			{
				...
				SharedMaterialDataStorage.allMaterials.Add(new SharedMaterialData(this,
					Submesh_info)); //and add to submesh list
				Submesh_sharedMaterial =
					SharedMaterialDataStorage.allMaterials[SharedMaterialDataStorage.allMaterials.Count - 1];
...
public static class SharedMaterialDataStorage
{
	...
	//	Debug.Log("Finding material for a submesh on " + stm.t.name);
		...
		//does this data have material data attached?
			if(!checkDefault && info.materialData != null)//it has material data?
			{ 
				...
				if(info.materialData.material.HasProperty(ShaderIds.Z_TEST_MODE))
				{
					if(MaterialExists_material.zTestMode != info.materialData.material.GetInt(ShaderIds.Z_TEST_MODE))
					{
						continue;
					}
				}
			...
			if(checkDefault) //check against values that matter on STM
			{
			...
			else //check against textinfo
			{
				...
				if(info.fontData != null)
				{
					...
					{
						if(info.isQuad &&
						   MaterialExists_material.refFilter != info.quadData.filterMode) //if filter mode doesn't match
						{
							continue;
						}
					}
				...
				else //no fontdata on the mesh?
				{
					...
					if(info.quadData != null) //if it has quad data 
					{
						...
						if(MaterialExists_material.refTex !=
						   info.quadData.texture) //if the two textures aren't the same...
						{
							//Debug.Log("info quad data was not null but material's reftex did not match the quad texture");
							continue;
							//safe = false;
						}
						//if ref tex == refmask, that's for non-silhouette quads. so if this quad *is* a silhouette...
						if((MaterialExists_material.refTex == MaterialExists_material.refMask) ==
						   (info.quadData.colorMode == STMQuadData.ColorMode.Silhouette)) //if they're not both a silhouette
						{
...
public class SharedMaterialData  //used internally by STM for keeping track of shared materials
{
	...
	public void SetValues(SuperTextMesh stm, STMTextInfo info)
	{
		...
		if(info.isQuad && info.quadData.colorMode != STMQuadData.ColorMode.Silhouette) //nah, use quad instead...
		{
	...
	public Material AsMaterial
	{
		get
		{
			...
			*/
			...
			materialName += "Base(" + (refMat != null ? refMat.name : "NULL MATERIAL") + ")";
			materialName += " - ";
			materialName += "UI(" + _stm.uiMode.ToString() + ")";
			materialName += " - ";
			if(refFont != null)
			{
				materialName += "Font(" + refFont.name + ")";
			}
			else
			{
				if(refTex != null)
				{
					materialName += "Quad(" + refTex.name + ")";
				}
				else
				{
					materialName += "NULL";
			...
			if(refMask != null)
			{
				materialName += "Mask(" + refMask.name + ")";
			}
			else
			{
				materialName += "Mask(NULL)";
			}
			materialName += " - ";
			materialName += "Filter(" + refFilter + ")";
			//materialName += " - ";
